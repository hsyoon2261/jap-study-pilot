...
JS=r"""() => {
  ...
  for(const e of document.querySelectorAll('*')){
    if((e.textContent||'').trim()==='szpdkss3.workers.dev' && e.children.length<=1){
      const r=e.getBoundingClientRect();
      if(r.width>10 && r.height>5){ val=e; break; }
    }
  }
  if(!val) return {err:'no visible val'};
  val.scrollIntoView({block:'center'});
  const vr=val.getBoundingClientRect(); const vy=vr.y+vr.height/2;
  let best=null;
  for(const s of document.querySelectorAll('svg')){
    const r=s.getBoundingClientRect();
    if(r.width>4 && r.width<32 && Math.abs((r.y+r.height/2)-vy)<16){
      if(best===null || r.x>best.x) best={x:r.x+r.width/2,y:r.y+r.height/2};
    }
  }
  return best||{err:'no pencil', vx:Math.round(vr.x), vy:Math.round(vy), vw:Math.round(vr.width)};
}"""
with attach_chrome() as (browser, ctx):
    ...
    snap(page,"cf_sub_edit9")
    body=page.inner_text("body")
    print("편집UI:", [k for k in ("available","Save","Cancel","Update","Register","subdomain","This action") if k in body])
