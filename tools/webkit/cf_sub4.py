...
JS=r"""() => {
  let val=null;
  for(const e of document.querySelectorAll('span,a,div')){
  ...
  for(const s of document.querySelectorAll('svg')){
    ...
    if(r.width>4 && Math.abs((r.y+r.height/2)-vy)<20 && r.x>=vr.x-4){
      if(!best || r.x>best.x) best={x:r.x+r.width/2,y:r.y+r.height/2};
    }
  }
  return best||{err:'no pencil'};
}"""
with attach_chrome() as (browser, ctx):
    ...
    snap(page,"cf_sub_edit7")
    body=page.inner_text("body")
    print("편집UI:", [k for k in ("available","Save","Cancel","Update","Register","Change subdomain","This action") if k in body])
